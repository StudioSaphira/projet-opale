...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('config-role-staff')
    .setDescription('Définit le rôle de staff général pour ce serveur')
    .addRoleOption(option =>
      option.setName('rôle')
        .setDescription('Le rôle à utiliser pour le staff général')
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const embed = createConfigEmbed('role_allstaff_id', role.id, user);
      await interaction.reply({ embeds: [embed], flags: 64 });

      await sendLogConfigToRubis(
        interaction.guild,
        interaction.user,
        `Le rôle staff général a été mis à jour : <@&${role.id}> (\`${role.id}\`)`,
        interaction.client,
        'Configuration : Rôles / Staff',
        '🛡️'
      );
    } catch (err) {
      console.error('[ERREUR /config-role-staff]', err);
