// bot/topaze/commands/config/channel/leaving.js
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('config-channel-leaving')
    .setDescription('Configurer le salon pour les messages de départ')
    .addChannelOption(option =>
      option
        .setName('salon')
        .setDescription('Le salon texte utilisé pour les messages de départ')
  ...
  async execute(interaction) {
    ...
    try {
      const current = db.prepare('SELECT channel_id FROM channel_leaving WHERE guild_id = ?').get(guildId);
      const oldChannelId = current?.channel_id;

      if (oldChannelId && oldChannelId !== selectedChannel.id) {
        db.prepare(`
          INSERT INTO channel_leaving (guild_id, old_channel_id)
          VALUES (?, ?)
          ON CONFLICT(guild_id) DO UPDATE SET old_channel_id = excluded.old_channel_id
        `).run(guildId, oldChannelId);

        logger.info(`[Topaze] Ancien salon de départ sauvegardé : ${oldChannelId}`);
      }

      db.prepare(`
        INSERT INTO channel_leaving (guild_id, channel_id)
        VALUES (?, ?)
        ON CONFLICT(guild_id) DO UPDATE SET channel_id = excluded.channel_id
      `).run(guildId, selectedChannel.id);

      const embed = createConfigEmbed('channel_leaving_id', selectedChannel.id, user);
      await interaction.reply({ embeds: [embed], flags: 64 });

      const logMessage = oldChannelId && oldChannelId !== selectedChannel.id
        ? `Le salon de départ a été mis à jour : <#${selectedChannel.id}> (\`${selectedChannel.id}\`) → Ancien : <#${oldChannelId}> (\`${oldChannelId}\`)`
        : `Le salon de départ a été défini : <#${selectedChannel.id}> (\`${selectedChannel.id}\`)`;
      ...
      logger.error(`[Topaze] Erreur DB – /config-channel-leaving : ${error.stack}`);
      return interaction.reply({
