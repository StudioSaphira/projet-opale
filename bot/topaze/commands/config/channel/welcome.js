// bot/topaze/commands/config/channel/welcome.js
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('config-channel-welcome')
    .setDescription('Configurer le salon pour les messages de bienvenue')
    .addChannelOption(option =>
      option
        .setName('salon')
        .setDescription('Le salon texte utilisé pour les messages de bienvenue')
  ...
  async execute(interaction) {
    ...
    try {
      const current = db.prepare('SELECT channel_id FROM channel_welcome WHERE guild_id = ?').get(guildId);
      const oldChannelId = current?.channel_id;

      if (oldChannelId && oldChannelId !== selectedChannel.id) {
        db.prepare(`
          INSERT INTO channel_welcome (guild_id, old_channel_id)
          VALUES (?, ?)
          ON CONFLICT(guild_id) DO UPDATE SET old_channel_id = excluded.old_channel_id
        `).run(guildId, oldChannelId);

        logger.info(`[Topaze] Ancien salon de bienvenue sauvegardé : ${oldChannelId}`);
      }

      db.prepare(`
        INSERT INTO channel_welcome (guild_id, channel_id)
        VALUES (?, ?)
        ON CONFLICT(guild_id) DO UPDATE SET channel_id = excluded.channel_id
      `).run(guildId, selectedChannel.id);

      const embed = createConfigEmbed('channel_welcome_id', selectedChannel.id, user);
      await interaction.reply({ embeds: [embed], flags: 64 });

      const logMessage = oldChannelId && oldChannelId !== selectedChannel.id
        ? `Le salon de bienvenue a été mis à jour : <#${selectedChannel.id}> (\`${selectedChannel.id}\`) → Ancien : <#${oldChannelId}> (\`${oldChannelId}\`)`
        : `Le salon de bienvenue a été défini : <#${selectedChannel.id}> (\`${selectedChannel.id}\`)`;
      ...
      logger.error(`[Topaze] Erreur DB – /config-channel-welcome : ${error.stack}`);
      return interaction.reply({
