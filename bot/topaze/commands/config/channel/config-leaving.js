// bot/topaze/commands/config/channel/config-leaving.js
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const row = db.prepare('SELECT role_admin_id, channel_leaving_id FROM server_config WHERE guild_id = ?').get(guildId);
    ...
    try {
      if (previousChannelId && previousChannelId !== channel.id) {
        db.prepare(`
          INSERT INTO old_server_config (guild_id, old_channel_leaving_id)
          VALUES (?, ?)
          ON CONFLICT(guild_id) DO UPDATE SET old_channel_leaving_id = excluded.old_channel_leaving_id
      ...
      const logMessage = previousChannelId
        ? `Le salon de départ a été mis à jour : <#${channel.id}> (\`${channel.id}\`) → <#${previousChannelId}> (\`${previousChannelId}\`)`
        : `Le salon de départ a été défini : <#${channel.id}> (\`${channel.id}\`)`;
