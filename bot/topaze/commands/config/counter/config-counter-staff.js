// bot/topaze/commands/config/counter/config-staff.js
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const config = db.prepare('SELECT role_admin_id, channel_counter_staff_id FROM server_config WHERE guild_id = ?').get(guildId);
    const adminRoles = config.role_admin_id?.split(',') || [];
    const hasAdminRole = adminRoles.some(id => interaction.member.roles.cache.has(id));
    const previousChannelId = config?.channel_counter_staff_id;
    ...
    try {
      if (previousChannelId && previousChannelId !== channel.id) {
        db.prepare(`
          INSERT INTO old_server_config (guild_id, old_channel_counter_staff_id)
          VALUES (?, ?)
          ON CONFLICT(guild_id) DO UPDATE SET old_channel_counter_staff_id = excluded.old_channel_counter_staff_id
      ...
      const logMessage = previousChannelId
        ? `Le compteur des membres du staff a été mis à jour : <#${channel.id}> (\`${channel.id}\`) → <#${previousChannelId}> (\`${previousChannelId}\`)`
        : `Le compteur des membres du staff a été défini : <#${channel.id}> (\`${channel.id}\`)`;
